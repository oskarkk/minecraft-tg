...
def to_tg(console, chat):
  ...
  for logType, logContent in logs.items():
    ...
    for lines in joinedLines:
      if logType == 'console':  # console log
        tg.send(conf.consoleID, lines)
      else:  # chat log
        tg.send(conf.chatID, lines, 'Markdown')
  ...
  if loginsAndLogouts:
    ...
    tg.send(conf.channelID, loginsAndLogouts)
    tg.chatTitle(conf.channelID, 'Wieliczka (' + str(len(usersOnline)) + ' online)')
